...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-base font-semibold text-gray-900">Associations</h1>
            <p class="mt-2 text-sm text-gray-700">A list of all the associations in your account.</p>
        </div>
            ...
            <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                <div class="overflow-hidden ring-1 shadow-sm ring-black/5 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tr>
                            <th scope="col"
                                class="py-3.5 pr-3 pl-4 text-left text-sm font-semibold text-gray-900 sm:pl-6">Account
                            </th>
                        ...
                        {#each associationsData.associations as association }
                            <tr>
                                <td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-6">
                                    {association.id?.account || 'n/a'}
                                </td>
                                <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{association.id?.cluster || 'n/a'}</td>
                                <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">
                                    {association.id?.user || 'n/a'}
                                </td>
                                <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{ association.id?.partition || 'n/a' }</td>
                                <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-6">
                                    <a href="associations/{association.id?.id}" class="text-indigo-600 hover:text-indigo-900">View</a>
                                </td>
                            </tr>
                        {/each}
                        <!-- More people... -->
