...
import { type UpdateQOSRequest } from '../gen/conclave/qos/v1/qos_service_pb';
import { type UpdateQOSResponse } from '../gen/conclave/qos/v1/qos_service_pb';
import {type QoS} from "../gen/conclave/qos/v1/qos_pb";
...
export async function fetchAssociations(): Promise<GetAssociationsResponse> {
	try {
		const request: GetAssociationsRequest = {
			$typeName: 'conclave.entities.v1.GetAssociationsRequest'
		};
...
export async function fetchAssociation(id: string): Promise<GetAssociationResponse> {
	try {
		const request: GetAssociationRequest = {
			$typeName: 'conclave.entities.v1.GetAssociationRequest',
			id
		};
...
export async function fetchPartitions(): Promise<GetPartitionsResponse> {
	try {
		const request: GetPartitionsRequest = {
			$typeName: 'conclave.partitions.v1.GetPartitionsRequest'
		};
...
export async function fetchPartition(name: string): Promise<GetPartitionResponse> {
	try {
		const request: GetPartitionRequest = {
			$typeName: 'conclave.partitions.v1.GetPartitionRequest',
			name
		};
...
export async function fetchComponentStatus(): Promise<GetComponentStatusResponse> {
	try {
		const request: GetComponentStatusRequest = {
			$typeName: 'conclave.cluster.v1.GetComponentStatusRequest'
		};
...
export async function updateQos(updatedQos: QoS): Promise<UpdateQOSResponse> {
	console.log('Sending QoS update to server:', updatedQos);

	// Make sure the message is part of an UpdateQOSRequest.
	// The $typeName is often required in reflection-based setups.
	const request: UpdateQOSRequest = {
		$typeName: 'conclave.qos.v1.UpdateQOSRequest',
		qos: updatedQos,
	};

	try {
		// Call the server's updateQOS RPC.
		return await qosService.updateQOS(request);
	} catch (err) {
		if (err instanceof ConnectError) {
			console.error('Failed to update QoS:', err.message, 'code:', err.code);
